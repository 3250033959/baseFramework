...
package com.msj.core.view.refreshLayout;
...
public class BGARefreshLayout extends LinearLayout {
    ...
    private void initRefreshHeaderView() {
        if(!mRefreshViewHolder.mIsRefreshingEnabled){
            return;
        }
        ...
        if (mRecyclerView != null) {
            mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
                ...
                public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                    if ((newState == RecyclerView.SCROLL_STATE_IDLE || newState == RecyclerView.SCROLL_STATE_DRAGGING || newState == RecyclerView.SCROLL_STATE_SETTLING) && shouldHandleRecyclerViewLoadingMore()) {
                         beginLoadingMore();
                    }
                }
            });
    ...
    private boolean shouldHandleRefresh() {
        ...
        if (mRecyclerView != null) {
            ...
            if (manager instanceof LinearLayoutManager) {
                LinearLayoutManager layoutManager = (LinearLayoutManager) manager;
                //之前的判定条件是&&改成了||是为了解决第一个item的高度大于屏幕，下拉刷新不成功
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (Math.abs(endX - startX) > 100) {
                        Log.e(LogConstant.RESULT_OPERA, "横距离超过100");
                        return true;
                    }
