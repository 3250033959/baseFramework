 ...
 */
package com.msj.core.view.scrollviewpager.bannerview;
...
//            int width_child = getChildAt(i).getWidth();
//            int height_child = getChildAt(i).getHeight();
//            System.out.println("--------------------"+width_child+"---------------"+height_child);
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		final int topY = locations[1];
		int bottonY = getHeight()+topY;
		if(ev.getRawY()>topY && ev.getRawY()< bottonY){
			isIntercepted = true;
		}
		switch (action) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			boolean xMoved = xDiff > 3;

			if (xMoved && isIntercepted) {
			...
			if (mTouchState == TOUCH_STATE_SCROLLING && isScrollable) {
	...
	protected void onScrollChanged(int h, int v, int oldh, int oldv) {
		...
		if (mIndicator != null) {
			...
			int hPerceived = h + (mCurrentAdapterIndex - mCurrentBufferIndex)
					* getWidth();
			//如果是最后一个向右滑动就不显示滑动轨迹,向左滑动就显示

			if(mCurrentBufferIndex != 0 && mCurrentAdapterIndex%mCurrentBufferIndex != mCurrentBufferIndex-1 || oldh>h){
