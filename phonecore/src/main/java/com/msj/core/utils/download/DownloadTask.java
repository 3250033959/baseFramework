package com.msj.core.utils.download;

import android.content.Context;
import android.os.Environment;
import android.widget.Toast;

import com.msj.core.utils.android.FileUtils;
import com.msj.core.utils.android.MemorySpaceUtils;
...
/**
 * 下载文件采用多个线程下载一个文件的方式
 * 使用直接new DownloadTask(url,threadNum,filePath,downloadListener,context)
...
public class DownloadTask extends Thread {
    ...
    public void run() {
        ...
        com.msj.core.utils.download.FileDownloadThread[] threads = new com.msj.core.utils.download.FileDownloadThread[threadNum];
        try {
            URL url = new URL(downloadUrl);
            LogUtils.e(LogConstant.DATA_OUTPUT, "download file http path:" + downloadUrl);
...
//            mProgressbar.setMax(fileSize);
            ...
            LogUtils.e(LogConstant.DATA_OUTPUT, "fileSize:" + fileSize + "  blockSize:"+blockSize);

            File file = new File(filePath);

            for (int i = 0; i < threads.length; i++) {
                // 启动线程，分别下载每个线程需要下载的部分
                threads[i] = new com.msj.core.utils.download.FileDownloadThread(url, file, blockSize,
                        (i + 1));
...
//                mHandler.sendMessage(msg);
            ...
            LogUtils.e(LogConstant.DATA_OUTPUT, " all of downloadSize:" + downloadedAllSize);
